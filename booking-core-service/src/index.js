...
const RABBITMQ_USER = process.env.RABBITMQ_USER; // Nowa zmienna
const RABBITMQ_PASS = process.env.RABBITMQ_PASS; // Nowa zmienna
const DB_HOST = process.env.DB_HOST || 'postgresql';
const DB_USER = process.env.DB_USER || 'user';
const DB_PASSWORD = process.env.DB_PASSWORD; // Odczytaj hasło
const DB_NAME = process.env.DB_NAME || 'booking_db';
const REDIS_HOST = process.env.REDIS_HOST || 'redis';
const REDIS_PORT = process.env.REDIS_PORT || 6379;
const REDIS_PASSWORD = process.env.REDIS_PASSWORD; // Odczytaj hasło
...
pgClient.connect()
    ...
    .catch(err => console.error('Core Service: Błąd połączenia z PostgreSQL', err));
...
async function connectRabbitMQ() {
    try {
        const connection = await amqp.connect(`amqp://${RABBITMQ_USER}:${RABBITMQ_PASS}@${RABBITMQ_HOST}`); // Użycie użytkownika i hasła
        ...
        setTimeout(connectRabbitMQ, 5000); // Ponowna próba połączenia po 5 sekundach
    }
}
connectRabbitMQ();
...
app.post('/book', async (req, res) => {
    // Przykładowa logika: publikacja zdarzenia do RabbitMQ
    if (channel) {
        const bookingData = { bookingId: Date.now(), item: 'Hotel Room', user: 'test@example.com' };
        channel.sendToQueue('booking_events', Buffer.from(JSON.stringify(bookingData)));
        res.status(200).json({ message: 'Zdarzenie rezerwacji wysłane!', data: bookingData });
    } else {
        res.status(500).json({ message: 'Błąd: Połączenie z RabbitMQ niedostępne.' });
    }
});
