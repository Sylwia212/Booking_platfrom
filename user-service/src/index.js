...
pgClient
  ...
  .catch((err) => {
    console.error(
      "User Service: KRYTYCZNY BŁĄD połączenia z PostgreSQL przy starcie:",
      err.stack
    );
  });
...
app.use((req, res, next) => {
  console.log(
    `USER-SERVICE: Incoming request: ${req.method} ${req.originalUrl}`
  ); 
  next();
});
...
app.get("/api/status", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: "User Service is running", db_status: "Connected" });
  } catch (dbError) {
...
app.post("/users/register", async (req, res) => {
  ...
  console.log("USER-SERVICE: Body received (parsed by express.json):", {
    email: email,
  ...
  try {
    ...
    console.log(
      "USER-SERVICE: User exists check completed. Rows:",
      userExists.rows.length
    );
...
app.post("/users/login", (req, res) => {
  console.log("USER-SERVICE: Handling /users/login (Not Implemented)");
...
app.use((err, req, res, next) => {
  console.error(
    "USER-SERVICE GLOBAL ERROR HANDLER:",
  ...
  if (res && !res.headersSent) {
    ...
    console.error(
      "USER-SERVICE GLOBAL ERROR HANDLER: Headers already sent, cannot send error response."
    );
    next(err);
  } else {
    console.error(
      "USER-SERVICE GLOBAL ERROR HANDLER: Response object is undefined."
    );
  }
});
